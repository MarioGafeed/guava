    ...
    <div class="row">
        <div class="col-md-12">
            <form action="">
                ...
                <input type="date" name="from" value="{{ request()->input('from') }}">
                <span>Check out before</span>
                <input type="date" name="to" value="{{ request()->input('to') }}">
                <button type="submit">filter</button>
                <button type="button" onclick="window.location = '{{ route('appartments.index') }}'">reset</button>
            </form>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <form method="post" action="{{ route('appartments.multi_delete') }}">
                @csrf
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.createBtn', function() {
            window.location = "{{ route('appartments.create') }}";
        });
